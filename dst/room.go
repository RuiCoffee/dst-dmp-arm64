...
func (g *Game) runningScreen() ([]string, error) {
	// 注意：不能使用固定列号（如原先的 $14）提取世界名，因为 ARM 架构下命令行前会被自动
	// 拼接 box64 前缀，导致 ps -ef 输出的字段整体错位；改为动态定位 "-shard" 参数后紧跟的值，
	// 无论是否存在 box64 前缀均可正确解析
	cmd := fmt.Sprintf(`ps -ef | grep 'DMP_Cluster_%d_' | grep dontstarve_dedicated_server_nullrenderer | grep -v grep | awk '{for(i=1;i<=NF;i++) if ($i=="-shard") print $(i+1)}'`, g.room.ID)
